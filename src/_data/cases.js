...
const cases = [
  {
    ...
    en: {
      ...
      approach:
        'The concierge runs on Cloudflare Workers with a Durable Object per conversation, so each visitor has isolated, consistent state without a database round trip for every turn. A second Durable Object enforces quota, because an agent with a language model behind it and no ceiling is an unbounded invoice. Leads land in D1 across three migrations — analytics, lead intelligence, retention runs — and three scheduled workflows enrich them, post a daily digest and run an audit pass. The admin dashboard reads the same store. What the agent never does is close the loop by itself: it drafts, it routes, it attaches context, and it hands a person a conversation that is already halfway understood.',
      result:
  ...
  },
  {
    ...
    en: {
      ...
      result:
        'The 3D now carries the whole scroll instead of the first screen, and the port removed more bytes than it added: 25 KB of OGL modules, gzipped, replacing 184 KB of three.js — measured across the full import closure, not the six files the source names. Two failures shipped and were caught in the process, both recorded in the commit history — a shader that compiled only on the fallback path, and an edge cache that served the broken version behind a fixed one.',
      ...
      proof: [
        { value: '25 KB', label: 'of renderer gzipped, replacing 184 KB of three.js' },
    ...
    es: {
      ...
      result:
        'El 3D ahora carga todo el scroll en lugar de la primera pantalla, y el puerto quitó más bytes de los que puso: 25 KB de módulos OGL, comprimidos, reemplazando 184 KB de three.js — medido sobre el cierre completo de imports, no los seis archivos que nombra el código. Dos fallos se publicaron y se cazaron en el proceso, ambos registrados en el historial — un shader que compilaba solo en la ruta de respaldo, y una caché de borde que sirvió la versión rota detrás de una ya arreglada.',
      ...
      stack: ['OGL', 'GLSL', 'GSAP ScrollTrigger', 'Lenis', 'Cloudflare Pages'],
      proof: [
        { value: '25 KB', label: 'de renderer comprimido, reemplazando 184 KB de three.js' },
